...
// A fake CLI whose basename drives profile detection.
function fakeCli(name, script) {
...
test('kubectl basename triggers kubernetes profile', () => {
  const kubectl = fakeCli('kubectl', `echo '${POD_JSON}'`)
...
test('gh basename triggers github profile with entity collapse', () => {
  const gh = fakeCli('gh', `echo '[{"number":5,"user":{"login":"octo","id":1},"node_id":"X","html_url":"h"}]'`)
...
test('unknown command falls back to generic (no pruning), --tj-profile overrides', () => {
  const mystery = fakeCli('mystery', `echo '{"node_id":"kept"}'`)
...
test('non-JSON output passes through byte-for-byte', () => {
  const plain = fakeCli('plain', `printf 'just some text\\n'`)
  const r = tj([plain])
...
test('non-zero exit: output untransformed, code propagated', () => {
  const bad = fakeCli('bad', `echo '{"message":"boom"}'; exit 3`)
  const r = tj([bad])
...
test('--tj-raw and --tj-json modes', () => {
  const gh = fakeCli('gh', `echo '{"node_id":"raw-kept"}'`)

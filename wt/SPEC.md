# Spec: wt — friendly git worktrees, per project
...
worktrees are actually in use. `wt` is a per-project CLI: run it inside any
...
2. "Agent is working on them" is detected heuristically: a `claude` / `node`
   ...
   best-effort, 4s timeout, skippable with `WT_NO_PROC=1`).
...
- `wt` / `wt list` — every worktree with branch, activity reasons
  (`dirty:N`, `unpushed:N`, `pr:#N`, `agent`, `locked`), PR number, path.
  Format: table on TTY, TOON piped, `--json` for scripts.
- `wt new [branch] [-m <note>] [--from <ref>]` — one command, idempotent:
  ...
  optional — generic names `wt-1`, `wt-2`, … are generated. `-m` stores a
  one-line intent note in `branch.<name>.description`. **stdout is the
  worktree path only** (so `cd "$(wt new x)"` works); friendly summary goes
  to stderr.
- `wt note [branch] [text]` — show or set the note; inside a worktree the
  branch is inferred (deepest containing worktree wins).
- `wt new --pr <n>` — fetches `pull/<n>/head` from origin (fork PRs work,
...
- The list's `work` column = stored note, else a summary generated live from
  ...
  "no work yet").
- `wt rm <branch|path> [--force]` — refuses to remove the main worktree;
  refuses dirty worktrees without `--force`; prunes; deletes merged branch.
- `wt clean [--yes]` — plans removal of all idle worktrees; executes only
  with `--yes`.
- `wt path <branch>` — prints the path (exit 1 if absent). Shell helper:
  `wtcd() { cd "$(wt new "$1")"; }`.
- Errors are one friendly line on stderr, exit 1; usage errors exit 2.
- Env: `WT_GH` (alternate gh binary; tests stub it), `WT_NO_PROC=1`.

## Project Structure

```
wt/
  bin/wt.js         → entry point
...
- E2E: temp git repos; create → list (TOON default when piped) → dirty →
  rm refusal → rm --force → clean dry-run/--yes; PR stub via fake `WT_GH`.
...
3. Piped `wt list` emits valid TOON; TTY shows the table.
4. `cd "$(wt new x)"` works (stdout purity).
